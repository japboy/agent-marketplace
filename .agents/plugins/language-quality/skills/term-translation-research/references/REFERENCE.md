...
Use this reference when a task requires a final term recommendation, a comparison of candidate
translations, or a decision about established usage.

## Source Hierarchy

Use the highest available source layer for the target audience. Do not let weak general usage
override strong domain or industry evidence.
...
- UNESCO, OECD, national ministries, and official curriculum glossaries for education terms.
- MDN, W3C, WHATWG, official API docs, or Context7-backed library docs for web and software terms.
...
For connected workspace sources, search internal pages before public web when the user links or
names a relevant internal source.
...
```markdown
**Decision** Use `<term>` for `<audience/context>`. Status: `<status>`, confidence: `<confidence>`.
...
For interchange with localization or translation systems, mention TBX only as a fundamental option
when the organization needs structured terminology exchange.
