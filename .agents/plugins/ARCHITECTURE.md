...
- Root `apm.yml` declares the catalog; both marketplace JSON files are generated by APM.
...
these states when changing a Plugin, Skill, catalog, or manifest. APM owns catalog generation and
its native drift gate; the repository integration test owns installation and Skill file
preservation. Other ownership and metadata equivalence rules remain architecture review contracts.
...
4. `Cataloged`: both marketplaces expose the same ordered Plugin set and point to the owning
   directories, and the native APM drift gate confirms they match `apm.yml`.
...
6. `Reviewed`: the complete change has been reviewed against this architecture contract, and the
   applicable Agent Skills, Codex Plugin, Claude strict validators, and APM integration gate pass.
...
3. Edit the ordered package declarations in root `apm.yml`, then regenerate both catalogs with `mise
   run marketplace-build`. Each source points to the owning `./.agents/plugins/<plugin>` directory.
   Format and commit both generated catalogs; do not edit them by hand.
...
Agent Skills validator owns Skill format validation. The ownership and cross-product metadata rules
above remain review-owned architecture contracts; generated catalog drift and APM installation are
