...
Use this skill to reason about route-state product analytics as a whole-system web architecture
problem, not as isolated frontend tracking calls.
...
- `not-applicable`: no catalogue entry matches. State that result and stop; do not turn the
  catalogue into generic web architecture advice.
