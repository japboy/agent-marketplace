...
> Reference:
> [State Machines - What's in a name?](https://stately.ai/blog/2024-01-23-state-machines-whats-in-a-name)
...
State machine definitions are declarative specifications, but transition tests are not inherently
redundant. Test externally meaningful transitions, final states, emitted events, and invoked side
effects when they protect behavior that could regress. A one-to-one restatement of a trivial
transition may be low value when stronger integration coverage already protects the same risk.
...
Use the [official XState testing guidance](https://stately.ai/docs/testing) as the baseline. Treat
the priorities above as team conventions, not as XState requirements.
